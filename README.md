...
- **Things GitHub rejects.** `hashFiles()` or `matrix` in a job-level `if:`, `branches` together with `branches-ignore`, a filter list with only negative patterns, unknown keys: the file is reported as GitHub would reject it and every job is marked not run, because that is what happens (the run shows up as a failed run named after the file, with no jobs).
